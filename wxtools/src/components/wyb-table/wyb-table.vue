	...
	<view class="wyb-table-box">
		...
		<view v-if="!loading" class="wyb-table-scroll-view" :style="{
			...
			<view class="wyb-table-header" :style="{borderBottom: '1px solid' + borderColor}">
				...
				<view class="wyb-table-header-item" v-for="(item, index) in headers" :key="item.key" @tap="onHeaderItemTap(index)"
				 :style="{
					 ...
					 <view class="wyb-table-header-icon" v-if="autoSortShow(index)">
					 	<text class="iconfont icon-arrow-up" :style="{
							...
							fontWeight: 'normal',
							marginBottom: fontSize[0] * -0.85 + 'rpx',
							transform: 'scale(0.4)'}" />
...
<script>
	import Pinyin from './characterToPinyin.js'
	import {isEqual} from './objEqual.js'
	export default {
		data() {
			return {
				bottomComputed: [],
